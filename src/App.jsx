    ...
    <div className="fade-in" style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
        {[
          ...
          <div key={label} style={{ background:bg, border:`1px solid ${border}`, borderRadius:14, padding:"14px 16px 16px", position:"relative" }}>
            <div style={{ position:"absolute", top:10, right:12, fontSize:22, opacity:0.12, pointerEvents:"none" }}>{icon}</div>
            <div style={{ fontSize:10, color:txt2, letterSpacing:1, marginBottom:8, fontWeight:500 }}>{label.toUpperCase()}</div>
            <div style={{ fontSize:22, fontFamily:"'Syne',sans-serif", fontWeight:800, color, lineHeight:1.3, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
      ...
      <div className="stats-3" style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
        {[["REGISTROS",regs.length,""],["TOTAL LITROS",fmtNum(regs.reduce((a,b)=>a+Number(b.quantidade||0),0)),"L"],["TOTAL GASTO",fmtBRL(regs.reduce((a,b)=>a+Number(b.custo||0),0)),""]].map(([label,val,unit]) => (
          <div key={label} style={{ background:"#1a1c27", border:"1px solid #2a2c3a", borderRadius:10, padding:"14px 18px" }}>
            ...
            <div style={{ fontSize:18, fontFamily:"'Syne',sans-serif", fontWeight:800, color:"#f97316", lineHeight:1.3, marginTop:4 }}>{val}{unit&&<span style={{fontSize:12,marginLeft:3,color:"#8a8a9a"}}>{unit}</span>}</div>
          </div>
        ))}
      </div>
              ...
              <div className="stats-3" style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
                {[["REGISTROS",regsSecretaria.length,""],["LITROS",fmtNum(regsSecretaria.reduce((a,b)=>a+Number(b.quantidade||0),0)),"L"],["GASTO",fmtBRL(regsSecretaria.reduce((a,b)=>a+Number(b.custo||0),0)),""]].map(([label,val,unit]) => (
                  <div key={label} style={{ background:"#1e3a2a", border:"1px solid #16a34a", borderRadius:10, padding:"12px 16px" }}>
                    ...
                    <div style={{ fontSize:16, fontFamily:"'Syne',sans-serif", fontWeight:800, color:"#4ade80", lineHeight:1.3, marginTop:4 }}>{val}{unit&&<span style={{fontSize:11,marginLeft:3}}>{unit}</span>}</div>
                  </div>
                ))}
              </div>
              ...
              <div className="stats-3" style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:16 }}>
                {[["REGISTROS",regsHistorico.length,""],["LITROS",fmtNum(regsHistorico.reduce((a,b)=>a+Number(b.quantidade||0),0)),"L"],["GASTO",fmtBRL(regsHistorico.reduce((a,b)=>a+Number(b.custo||0),0)),""]].map(([label,val,unit]) => (
                  <div key={label} style={{ background:"#1a1c27", border:"1px solid #2a2c3a", borderRadius:10, padding:"12px 16px" }}>
                    ...
                    <div style={{ fontSize:16, fontFamily:"'Syne',sans-serif", fontWeight:800, color:"#38bdf8", lineHeight:1.3, marginTop:4 }}>{val}{unit&&<span style={{fontSize:11,marginLeft:3,color:"#5a5a6a"}}>{unit}</span>}</div>
                  </div>
                ))}
              </div>
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                ...
                <div>
                  ...
                  <div className="stats-3" style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:20 }}>
                    <div style={{ background:"#1a1c27", border:"1px solid #2a2c3a", borderRadius:10, padding:"14px 18px" }}>
                      ...
                      <div style={{ fontSize:18, fontFamily:"'Syne',sans-serif", fontWeight:800, color:"#f97316", lineHeight:1.3, marginTop:4 }}>{fmtBRL(mesAtual.custo)}</div>
                    ...
                    <div style={{ background:"#1a1c27", border:"1px solid #2a2c3a", borderRadius:10, padding:"14px 18px" }}>
                      ...
                      <div style={{ fontSize:18, fontFamily:"'Syne',sans-serif", fontWeight:800, color:"#8a8a9a", lineHeight:1.3, marginTop:4 }}>{fmtBRL(mesAnterior.custo)}</div>
                    ...
                    <div style={{ background: variacao > 0 ? "#2d0f0f" : "#14532d", border:`1px solid ${variacao > 0 ? "#ef4444" : "#16a34a"}`, borderRadius:10, padding:"14px 18px" }}>
                      ...
                      <div style={{ fontSize:18, fontFamily:"'Syne',sans-serif", fontWeight:800, color: variacao > 0 ? "#ef4444" : "#4ade80", lineHeight:1.3, marginTop:4 }}>
          ...
          <div className="fade-in">
            <div className="stats-3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 6, marginBottom: 16 }}>
              {[["REGISTROS", filtered.length, ""], ["TOTAL LITROS", fmtNum(filtered.reduce((a, b) => a + Number(b.quantidade || 0), 0)), "L"], ["TOTAL GASTO", fmtBRL(filtered.reduce((a, b) => a + Number(b.custo || 0), 0)), ""]].map(([label, val, unit]) => (
                <div key={label} style={{ background: "#1a1c27", border: "1px solid #2a2c3a", borderRadius: 10, padding: "10px 10px 12px" }}>
                  <div style={{ fontSize: 9, color: "#5a5a6a", letterSpacing: 1, marginBottom:4 }}>{label}</div>
                  <div style={{ fontSize: 14, fontFamily: "'Syne',sans-serif", fontWeight: 800, color: "#f97316", lineHeight:1.3, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{val}{unit && <span style={{ fontSize: 10, marginLeft: 2, color: "#8a8a9a" }}>{unit}</span>}</div>
                </div>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        {!loading && activeTab === "meus-registros" && (
          <div className="fade-in">
            <div style={{ background:"#1a1c27", border:"1px solid #2a2c3a", borderRadius:10, padding:"14px 18px", marginBottom:16 }}>
              ...
              <div style={{ fontSize:20, fontFamily:"'Syne',sans-serif", fontWeight:800, color:"#f97316", lineHeight:1.3, marginTop:4 }}>
